...
\begin{frame}[fragile]%{Linux内核关键数据结构：list}
\frametitle{Linux内核关键数据结构：双向循环链表(list)}
\lstfox
\begin{block}{思考：}
这样定义链表有什么主要缺点?
\end{block}
...
%\lstlistheadusage
与上页中的fox结构相比，这样定义的list\_head能有什么用处?
\end{block}
\end{frame}

\begin{frame}[fragile]%{Linux内核关键数据结构：list}
\frametitle{Linux内核关键数据结构}
\begin{block}{list\_head的使用方法举例: include/linux/sched.h}
\lstlistheadusage
\end{block}
\begin{block}{思考}
%\begin{itemize}
%\item 
仍与fox结构相比，使用list\_head构造链表有什么好处?
%\end{itemize}
\end{block}
\end{frame}
...
练习：请在该头文件中找到container\_of的定义并试着理解它。
\end{block}
\end{frame}
...
\item \alert{list\_for\_each\_entry}(pos, head) 
\end{itemize}
\end{frame}
...
\begin{frame}{fragile}
\frametitle{Linux进程描述符: Process Descriptor}
\lstenqueue
\begin{block}{特别注意}
